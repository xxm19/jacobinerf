...
def sampling_index(n_rays, batch_size, h, w, gt_sem=None):
    ...
    if gt_sem is not None:
        ...
        mask_all = np.array(gt_sem>=0)
        # TODO: multiview
        labels = np.unique(gt_sem[mask_all])
        num_class = len(labels)
        index_hw = []
        for l in labels:
            mask = np.array(gt_sem == l)
            # index_hw.append(torch.arange(h*w)[mask][torch.randint(0, mask.sum(), (1, int(n_rays/num_class)))])
            sampled_ids = np.random.choice(np.arange(mask.sum()), (1, min(int(n_rays/num_class), mask.sum())), replace=False)
            index_hw.append(np.arange(h*w)[mask][sampled_ids])
            mask_all[sampled_ids] = False
        num = np.concatenate(index_hw, -1).shape[-1]
        if num < n_rays:
            sampled_ids = np.random.choice(np.arange(mask_all.sum()), (1, n_rays-num), replace=False)
            index_hw.append(np.arange(h*w)[mask_all][sampled_ids])
        index_hw = np.concatenate(index_hw, -1)
    else:
        # index_hw = torch.randint(0, h * w, (1, n_rays))
        index_hw = np.random.choice(np.arange(h*w), (1, n_rays), replace=False)
