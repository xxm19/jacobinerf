...
from training.training_utils import batchify
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, endpoint_feat=False, raw_semantic=None, pts_sampled=None):
    ...
    if raw_semantic is not None:
        sem_map = torch.sum(weights[..., None] * raw_semantic, -2)  # [N_rays, num_class]
        return sem_map

    rgb_map = torch.sum(weights[..., None] * rgb, -2)  # [N_rays, 3]
    # [N_rays, 3], the accumulated opacity along the rays, equals "1 - (1-a1)(1-a2)...(1-an)" mathematically

    if pts_sampled is not None:
        xyz_map = torch.sum(weights[..., None] * pts_sampled, -2)   # [N_rays, 3]
    else:
        xyz_map = torch.tensor(0)
    ...
    return rgb_map, disp_map, acc_map, weights, depth_map, feat_map, xyz_map
